...
const BeamsScene = ({
  ...
  return (
    <group rotation={[degToRad(-10), 0, degToRad(rotation)]}>
      <group position={[0, 0, -10]}>
        <MergedPlanes ref={meshRef} material={beamMaterial} width={beamWidth} count={beamNumber} height={beamHeight} />
        <DirLight position={[0, 5, 2]} color={lightColor} />
      </group>
...
const DirLight = ({ position, color }: DirLightProps) => {
  ...
  return <directionalLight ref={dir} position={position} color={color} intensity={2} />;
};

const Beams = (props: BeamsProps) => {
  return (
    <div className="beams-container">
      <Canvas className="beams-canvas" gl={{ antialias: true, alpha: true }}>
        <BeamsScene {...props} />
      </Canvas>
    </div>
  );
